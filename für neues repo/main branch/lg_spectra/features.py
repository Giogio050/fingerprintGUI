...
def _weighted_median(x: np.ndarray, w: np.ndarray) -> float:
    ...
    return float(np.interp(0.5, cdf, x_sorted))
...
def compute_features(
    ...
    *,
    ...
    ----------
    lam, y
        Wavelength grid and processed spectral intensities.
    sticks
        Stick representation of peaks.
    rt_min
        Optional retention time in minutes to embed into the fingerprint.
    meta
        Optional metadata dictionary describing the spectrum origin.
    """
    ...
    feats = {
        ...
        'global': {
            ...
            'lambda_median': _weighted_median(lam, y_norm),
    ...
    feats['rt_min'] = float(rt_min) if rt_min is not None else 0.0
    if meta:
        feats['meta'] = dict(meta)
    return feats
